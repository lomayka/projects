...
static cJSON * _programmer_serializeJSON(const programmer_t * self) {
	...
	cJSON_AddItemToObject(jProgrammer, "birthdate", cJSON_CreateString(self->data_birth));
	cJSON * jCompany = cJSON_CreateObject();
	cJSON_AddItemToObject(jCompany, "company_name", cJSON_CreateString(self->company.name));
