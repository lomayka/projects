...
int count(int size, struct Factories arr[size]){
    int cnt = 0;
...
void change(struct Factories * pVar, const char * newValue){
    strcpy(pVar->name, newValue);
}
void print(struct Factories * pVar){
    for (int i = 0; i < 4; i++){
...
{
   srand(time(NULL));
   struct Factories arr[4] = {{rand()%100+1, "Dell", "Tom Johns"},{rand()%100+1,"Asus", "Peter Cooper"}, {rand()%100+1,"MSI", "Henry Parker"}, {rand()%100+1,"HP", "Steve MacGee"}};
    int N = 0;
    char * newValue;
